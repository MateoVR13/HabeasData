...
const InfoCard: React.FC<{ icon: React.ReactNode; title: string; text: string; delay: number }> = ({ icon, title, text, delay }) => (
    <div className="bg-slate-800 p-6 rounded-lg shadow-lg transform hover:-translate-y-2 transition-transform duration-300 animate-fade-in-up" style={{ animationDelay: `${delay}ms` }}>
        <div className="flex items-center mb-3">
            ...
            <h3 className="text-xl md:text-2xl font-bold text-cyan-400">{title}</h3>
        </div>
...
const Slide2: React.FC = () => {
    return (
        <div className="h-full w-full flex flex-col items-center justify-center p-4 sm:p-6 md:p-8 animate-fade-in overflow-y-auto">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-12 text-center animate-slide-down">¿Qué es el Habeas Data?</h1>
            ...
            <p className="text-slate-400 mt-12 text-center max-w-4xl text-base md:text-lg animate-fade-in" style={{ animationDelay: '1000ms' }}>
